...
- `doc/usage_osc_receiver.gif` - OSCレシーバーの使用方法
- `src/oscReceiver/data/config.json` - OSCポート設定
- `src/oscReceiver/oscReceiver.pde` - メインアプリケーションコード
- `src/oscReceiver/GuiListener.pde` - GUIイベントハンドラー
...
### OSCレシーバーアプリケーション
- **技術スタック**: Processing/Java, Swing/AWT, OSC, Log4j
- **主要機能**: OSCメッセージの受信・表示、GUIインターフェース
- **設定**: JSONファイルによるポート設定（デフォルト: 8081）
- **ログ**: Log4jによる構造化ログ出力

### 開発環境
- **IDE**: Processing IDE
- **依存ライブラリ**: oscP5.jar, log4j.jar
- **設定ファイル**: log4j.properties, config.json
